-- search_recipes previously received p_pantry_roots and p_expiring_roots from the client.
-- Now it reads the calling user's pantry directly via auth.uid(), so the client no longer
-- needs to compute or pass those lists.
-- The chain pantry_item -> ingredient -> root is at most one level deep (parent_ingredient_id),
-- so a single self-join handles root resolution. Items expiring within 3 days (0-3 days left)
-- map to CRITICAL or WARNING on the client, so that is the threshold used here too.
DROP FUNCTION IF EXISTS public.search_recipes(TEXT, TEXT[], TEXT[], TEXT, TEXT[], UUID[], UUID[], UUID[], INT, INT, TEXT, INT);
